...
class Manager(object):
    ...
    def add_host(self, host, port=22, user=None, password=None):
        ...
        try:
            ...
            raise ValueError(
                "[{}] Invalid option for port. Must be integer: {}".format(host, port)
            )

        if host not in self.manager:
            self.manager[host] = {}
        self.manager[host].update({"port": port, "user": user, "pass": password})
    ...
    def get_ssh_connection(self, host):
        ...
        if con is None:
            try:
                ...
                if E.errno == -2:
                    raise ValueError(
                        "{}: Invalid host given '{}'. Make sure your host is valid".format(
                            str(E), host
                        )
                    )
